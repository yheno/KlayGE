...
namespace KlayGE
{
	...
	void D3D12Texture3D::CopyToTexture(Texture& target)
	{
		...
		D3D12Texture3D& other(*checked_cast<D3D12Texture3D*>(&target));
	...
	ID3D11ShaderResourceViewPtr const & D3D12Texture3D::RetriveD3DShaderResourceView(uint32_t first_array_index, uint32_t num_items, uint32_t first_level, uint32_t num_levels)
	{
	...
	ID3D11UnorderedAccessViewPtr const & D3D12Texture3D::RetriveD3DUnorderedAccessView(uint32_t first_array_index, uint32_t num_items, uint32_t level)
	{
	...
	ID3D11UnorderedAccessViewPtr const & D3D12Texture3D::RetriveD3DUnorderedAccessView(uint32_t array_index, uint32_t first_slice, uint32_t num_slices, uint32_t level)
	{
	...
	ID3D11RenderTargetViewPtr const & D3D12Texture3D::RetriveD3DRenderTargetView(uint32_t array_index, uint32_t first_slice, uint32_t num_slices, uint32_t level)
	{
	...
	ID3D11DepthStencilViewPtr const & D3D12Texture3D::RetriveD3DDepthStencilView(uint32_t array_index, uint32_t first_slice, uint32_t num_slices, uint32_t level)
	{
		BOOST_ASSERT(this->AccessHint() & EAH_GPU_Write);
		BOOST_ASSERT(0 == array_index);
		UNREF_PARAM(array_index);
			...
			void*& data, uint32_t& row_pitch, uint32_t& slice_pitch)
	{
		...
		TIF(d3d_imm_ctx_->Map(d3dTexture3D_.get(), D3D11CalcSubresource(level, array_index, num_mip_maps_), D3D12Mapping::Mapping(tma, type_, access_hint_, num_mip_maps_), 0, &mapped));
	...
	void D3D12Texture3D::OfferHWResource()
	{
		d3d_sr_views_.clear();
		d3d_ua_views_.clear();
		d3d_rt_views_.clear();
	...
	void D3D12Texture3D::ReclaimHWResource(ElementInitData const * init_data)
	{
