...
namespace
{
	...
	class SetD3D12ShaderParameterSampler
	{
	public:
		SetD3D12ShaderParameterSampler(ID3D11SamplerStatePtr& sampler, RenderEffectParameterPtr const & param)
		...
		void operator()()
		{
			...
			if (sampler)
			{
				*sampler_ = checked_cast<D3D12SamplerStateObject*>(sampler.get())->D3DSamplerState();
			}
		}
	...
	{
	public:
		SetD3D12ShaderParameterTextureSRV(tuple<void*, uint32_t, uint32_t>& srvsrc, ID3D11ShaderResourceViewPtr& srv, RenderEffectParameterPtr const & param)
	...
	{
	public:
		SetD3D12ShaderParameterGraphicsBufferSRV(tuple<void*, uint32_t, uint32_t>& srvsrc, ID3D11ShaderResourceViewPtr& srv, RenderEffectParameterPtr const & param)
			: srvsrc_(&srvsrc), srv_(&srv), param_(param)
		{
		}
	...
	{
	public:
		SetD3D12ShaderParameterTextureUAV(void*& uavsrc, ID3D11UnorderedAccessViewPtr& uav, RenderEffectParameterPtr const & param)
	...
	{
	public:
		SetD3D12ShaderParameterGraphicsBufferUAV(void*& uavsrc, ID3D11UnorderedAccessViewPtr& uav, RenderEffectParameterPtr const & param)
			: uavsrc_(&uavsrc), uav_(&uav), param_(param)
		{
		}
...
namespace KlayGE
{
	...
	shared_ptr<std::vector<uint8_t> > D3D12ShaderObject::CompiteToBytecode(ShaderType type, RenderEffect const & effect,
						...
						else
						{
							...
							if (!sd.so_decl.empty())
							{
								if (caps.gs_support)
								{
									...
									{
										d3d11_decl[i] = D3D12Mapping::Mapping(sd.so_decl[i], static_cast<uint8_t>(i));
									}
	...
	void D3D12ShaderObject::LinkShaders(RenderEffect const & effect)
	{
		...
		for (size_t type = 0; type < ShaderObject::ST_NumShaderTypes; ++ type)
		{
			...
			for (size_t i = 0; i < cbuff_indices_[type].size(); ++ i)
			{
				...
				d3d11_cbuffs_[type][i] = checked_cast<D3D12GraphicsBuffer*>(cbuff->HWBuff().get())->D3DBuffer();
			}
		}
	...
	ShaderObjectPtr D3D12ShaderObject::Clone(RenderEffect const & effect)
	{
		...
		for (size_t i = 0; i < ST_NumShaderTypes; ++ i)
		{
			...
			for (size_t j = 0; j < cbuff_indices_[i].size(); ++ j)
			{
				...
				ret->d3d11_cbuffs_[i][j] = checked_cast<D3D12GraphicsBuffer*>(cbuff->HWBuff().get())->D3DBuffer();
			}
	...
	D3D12ShaderObject::parameter_bind_t D3D12ShaderObject::GetBindFunc(D3D12ShaderParameterHandle const & p_handle, RenderEffectParameterPtr const & param)
	{
		...
		case REDT_sampler:
			ret.func = SetD3D12ShaderParameterSampler(samplers_[p_handle.shader_type][p_handle.offset], param);
			break;
