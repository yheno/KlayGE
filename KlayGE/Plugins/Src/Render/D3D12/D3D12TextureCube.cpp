...
namespace KlayGE
{
	...
	void D3D12TextureCube::CopyToTexture(Texture& target)
	{
		...
		D3D12TextureCube& other(*checked_cast<D3D12TextureCube*>(&target));
	...
	ID3D11ShaderResourceViewPtr const & D3D12TextureCube::RetriveD3DShaderResourceView(uint32_t first_array_index, uint32_t num_items, uint32_t first_level, uint32_t num_levels)
	{
	...
	ID3D11UnorderedAccessViewPtr const & D3D12TextureCube::RetriveD3DUnorderedAccessView(uint32_t first_array_index, uint32_t num_items, uint32_t level)
	{
		return this->RetriveD3DUnorderedAccessView(first_array_index, num_items, CF_Positive_X, 6, level);
	}

	ID3D11UnorderedAccessViewPtr const & D3D12TextureCube::RetriveD3DUnorderedAccessView(uint32_t first_array_index, uint32_t num_items, CubeFaces first_face, uint32_t num_faces, uint32_t level)
	{
		BOOST_ASSERT(this->AccessHint() & EAH_GPU_Read);
	...
	ID3D11RenderTargetViewPtr const & D3D12TextureCube::RetriveD3DRenderTargetView(uint32_t first_array_index, uint32_t array_size, uint32_t level)
	{
	...
	ID3D11RenderTargetViewPtr const & D3D12TextureCube::RetriveD3DRenderTargetView(uint32_t array_index, Texture::CubeFaces face, uint32_t level)
	{
		BOOST_ASSERT(this->AccessHint() & EAH_GPU_Write);
	...
	ID3D11DepthStencilViewPtr const & D3D12TextureCube::RetriveD3DDepthStencilView(uint32_t first_array_index, uint32_t array_size, uint32_t level)
	{
		BOOST_ASSERT(this->AccessHint() & EAH_GPU_Write);
		BOOST_ASSERT(first_array_index < this->ArraySize());
		BOOST_ASSERT(first_array_index + array_size <= this->ArraySize());
	...
	ID3D11DepthStencilViewPtr const & D3D12TextureCube::RetriveD3DDepthStencilView(uint32_t array_index, Texture::CubeFaces face, uint32_t level)
	{
		BOOST_ASSERT(this->AccessHint() & EAH_GPU_Write);
			...
			void*& data, uint32_t& row_pitch)
	{
		...
		TIF(d3d_imm_ctx_->Map(d3dTextureCube_.get(), D3D11CalcSubresource(level, array_index * 6 + face - CF_Positive_X, num_mip_maps_),
			D3D12Mapping::Mapping(tma, type_, access_hint_, num_mip_maps_), 0, &mapped));
	...
	void D3D12TextureCube::OfferHWResource()
	{
		d3d_sr_views_.clear();
		d3d_ua_views_.clear();
		d3d_rt_views_.clear();
	...
	void D3D12TextureCube::ReclaimHWResource(ElementInitData const * init_data)
	{
